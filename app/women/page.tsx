...
function ProductCard({ product }: { product: any }) {
  ...
  const fakeMrp = Math.round(Number(product.price || 0) * 1.35);
      ...
      <Link href={`/${slug}/${product.id}`}>
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-sm text-zinc-500 line-through">₹{fakeMrp}</span>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {Object.entries(product.sizes || {})
              ...
              .map(([size, qty]) => (
                <span
                  key={size}
                  className="rounded-full border border-white/10 bg-black px-3 py-1 text-xs text-zinc-300"
                >
                  {size}: {Number(qty)}
                </span>
              ))}
          ...
          <p className="mt-3 text-sm text-zinc-400">Stock: {totalStock}</p>
        </div>
      </Link>
    </div>
  );
    ...
    const loadProducts = async () => {
      try {
        ...
        const womenProducts = (data || []).filter(
          (item: any) => String(item.category || "").toLowerCase() === "women"
        );
  ...
  return (
    <div className="min-h-screen bg-black px-4 py-8 text-white sm:px-6 sm:py-10">
      <div className="mx-auto max-w-7xl">
        <p className="text-sm uppercase tracking-[0.35em] text-zinc-500">Women</p>
        <h1 className="mt-3 text-4xl font-bold sm:text-5xl">Women Collection</h1>
        <p className="mt-4 max-w-2xl text-zinc-400">
          Browse tops, jeans, dresses, and more from your women collection.
        </p>
