    ...
    const loadProducts = async () => {
      try {
        ...
        const shoesProducts = (data || []).filter(
          (item: any) => String(item.category || "").toLowerCase() === "shoes"
        );
        setProducts(shoesProducts);
      } catch (error) {
        console.error("Failed to load shoes products:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-black px-4 py-8 text-white sm:px-6 sm:py-10">
      <div className="mx-auto max-w-7xl">
        <p className="text-sm uppercase tracking-[0.35em] text-zinc-500">Shoes</p>
        <h1 className="mt-3 text-4xl font-bold sm:text-5xl">Shoes Collection</h1>
        <p className="mt-4 max-w-2xl text-zinc-400">
          Browse sneakers, heels, sandals, flats, and more from your shoes collection.
        </p>
